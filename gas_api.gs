...
// ─ タスクボードをサンプルデータ付きで作成（GASエディタで1回実行） ─
function createTaskBoardSample() {
  // まずシート構造を作成
  setupTaskBoard();
  ...
  // サンプルデータ: [ID, 登録日, カテゴリ, タスク名, 担当者, 期限, ステータス, 優先度, 備考, 完了日]
  const samples = [
    [1,  "2026/03/17", "集客",    "加盟店買収リストの精査決定",                     "天龍",     "2026/03/18", "進行中",   "A（今月）",   "",                       ""],
    [2,  "2026/03/17", "集客",    "立川/藤沢で回数券について配信",                   "タカショー","2026/03/19", "確認待ち", "S（今週必須）","藤沢配信完了/立川23日以降に配信を実施", ""],
    [3,  "2026/03/17", "",        "カウンセリングシートに来店周期についての記載を追加し、お客様に説明", "タカショー","2026/03/20", "未着手",   "A（今月）",   "",                       ""],
    [4,  "2026/03/20", "集客(HPB・LINE)", "平日限定クーポン5種類作成し、店舗状況路まえ掲載する:most", "天龍","2026/03/20", "未着手",   "",            "",                       ""],
    [5,  "2026/03/17", "",        "お客様への遅刻厳禁アナウンス",                     "天龍",     "2026/03/20", "未着手",   "",            "所沢店",                  ""],
    [6,  "2026/03/17", "",        "新規限定の物販販売の件数集計",                     "天龍",     "2026/03/20", "未着手",   "",            "LUMISS含む",             ""],
    [7,  "2026/03/17", "",        "新規予約数の集計（PV数に対する予約率/予約間隔）",   "天龍",     "2026/03/20", "進行中",   "",            "推奨CSVデータ使用",       ""],
    [8,  "2026/03/19", "新規数",  "ミニモの店舗アカウント運用開始",                   "タカショー","2026/03/21", "未着手",   "A（今月）",   "",                       ""],
    [9,  "2026/03/17", "売上施策", "エニシーグローバック：動画作成+資料共有でスタッフ落とし込み", "メイメイ","2026/03/21", "未着手",   "",            "",                       ""],
    [10, "2026/03/17", "",        "アフターケアの時間確保",                           "天龍",     "2026/03/21", "未着手",   "",            "所沢店。カウンセリングシートに記載あり", ""],
    [11, "2026/03/20", "事業計画", "事業計画に営業利益・営業利益目標を作成",           "天龍",     "2026/03/29", "未着手",   "S（今週必須）","全体の営業利益を今期どのくらいで調整し、どのくらいまで投資できるか。", ""],
    [12, "2026/03/17", "売上施策", "most立川にメニューを追加する（マツエクの）",       "天龍",     "2026/03/21", "未着手",   "A（今月）",   "",                       ""],
    [13, "2026/03/20", "事業計画", "売却について舘泉さんに確認",                       "あきほさん","2026/03/21", "未着手",   "S（今週必須）","",                       ""],
  ];

  ws.getRange(2, 1, samples.length, 10).setValues(samples);

  // 日付列のフォーマット再適用
  ws.getRange(2, 2, samples.length, 1).setNumberFormat("yyyy/MM/dd");
  ws.getRange(2, 6, samples.length, 1).setNumberFormat("yyyy/MM/dd");

  Logger.log("タスクボード作成完了：" + samples.length + "件のサンプルデータを追加しました。");
  SpreadsheetApp.getActiveSpreadsheet().toast("タスクボード作成完了！", "完了", 3);
}
